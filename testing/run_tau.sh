...
output_dir_array=("heat_shock_10x" "schlogl_output_case2-" "stochkit_events_output_case2-")
...
do
    ...
    do
       ...
       do
          ...
          ./tau_leaping -m "models/examples/$model_item" -t 2 -r 2 $interval_item --out-dir "models/examples/$output_dir_item$count_pad" "$output_item" -f --seed "$seed_item" >"$cur_dir/logs_tau/case$count_pad/stdout2.txt" 2>&1
          let count=count+1
       done
    done
done
