...
models_array=("heat_shock_10x.xml" "schlogl.xml" "istochkit_events.xml")
...
do
    ...
    do
       ...
       do
          ...
          ./tau_leaping -m "models/examples/$model_item" -t 2 -r 2 $interval_item --out-dir "models/examples/$output_dir_item$count_pad" "$output_item" -f --seed "$seed_item" -p 7 >"$cur_dir/logs_tau/case$count_pad/stdout3.txt" 2>&1
          let count=count+1
